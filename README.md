...
A montagem elétrica do protótipo reúne a Raspberry Pi, o ESP32, as pontes H, os
motores com encoder, o sensor MPU6050, a câmera USB e a alimentação do sistema. O
diagrama abaixo serve como referência para entender as ligações físicas entre os
módulos e facilitar a montagem, manutenção e depuração do robô.
...
### 4.1 Navegador ⇄ Raspberry Pi (WebSocket / Socket.IO)
...
### 4.2 Raspberry Pi ⇄ ESP32 (serial USB, JSON por linha)
...
## 5. Visão computacional (AprilTag)
...
## 6. Controle PID dos motores (no ESP32)
...
## 7. A lógica da busca automática (máquina de estados)
...
### 7.1 Os quatro estados conceituais
...
### 7.2 Como isso aparece no código
...
├── docs/
│   └── images/
│       └── diagrama-eletrico.jpg  # Foto do diagrama elétrico da montagem
